/*
 * Ed25519 digital signatures and X25519 key agreement — extracted from TweetNaCl.
 *
 * Only the functions required for Ed25519 sign / verify / seed-based keypair
 * generation and X25519 scalar multiplication are included. Everything else
 * (Salsa20, Poly1305, secretbox, crypto_box, etc.) has been removed.
 *
 * TweetNaCl's built-in SHA-512 has been replaced with mbedTLS's
 * mbedtls_sha512() to avoid duplicating a SHA-512 implementation that
 * the project already provides via its mbedTLS dependency.
 *
 * The code below is copied verbatim from TweetNaCl 20140427 by
 ...
 *  - crypto_sign_seed_keypair(), the standard seed-based variant of
 *    crypto_sign_keypair (same as libsodium provides).
...
int crypto_sign_ed25519_seed_keypair(unsigned char pk[32], unsigned char sk[64], const unsigned char seed[32]) {
    u8 d[64];
    gf p[4];
    int i;
