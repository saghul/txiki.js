...
const path = {
    ...
    join(...args) {
        ...
        if (isPathSeparator(StringPrototypeCharCodeAt(firstPart, 0))) {
            ...
            if (
                ...
                if (firstLen > 2) {
                    if (
                        isPathSeparator(StringPrototypeCharCodeAt(firstPart, 2))
                    ) {
                        ++slashCount;
                    } else {
    ...
    relative(from, to) {
        ...
        const fromOrig = path.resolve(from);
        const toOrig = path.resolve(to);
    ...
    toNamespacedPath(path) {
        ...
        if (
            StringPrototypeCharCodeAt(resolvedPath, 0) === CHAR_BACKWARD_SLASH
        ) {
            // Possible UNC root
            if (
    ...
    dirname(path) {
        ...
        if (isPathSeparator(code)) {
            ...
            rootEnd =
                len > 2 && isPathSeparator(StringPrototypeCharCodeAt(path, 2))
                    ? 3
                    : 2;
            offset = rootEnd;
        }
