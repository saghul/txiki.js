...
// Regression test: an uncaught top-level exception with a live fswatch handle
...
const args = [
    tjs.exePath,
    'run',
    path.join(import.meta.dirname, 'helpers', 'teardown-live-fswatch.js')
];
const proc = tjs.spawn(args, { stdout: 'ignore', stderr: 'pipe' });
const [ status, stderrStr ] = await Promise.all([ proc.wait(), proc.stderr.text() ]);
assert.ok(stderrStr.includes('uncaught with an active fswatch'), 'child threw the expected error');
