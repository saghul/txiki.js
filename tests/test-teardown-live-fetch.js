...
// Regression test: an uncaught top-level exception with a live fetch handle
...
const args = [
    tjs.exePath,
    'run',
    path.join(import.meta.dirname, 'helpers', 'teardown-live-fetch.js')
];
const proc = tjs.spawn(args, { stdout: 'ignore', stderr: 'pipe' });
const [ status, stderrStr ] = await Promise.all([ proc.wait(), proc.stderr.text() ]);
assert.ok(stderrStr.includes('uncaught with an in-flight fetch'), 'child threw the expected error');
