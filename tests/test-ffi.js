...
import initCParser from '../src/js/core/tjs/utils/ffiutils.js';
...
(function () {
    ...
    function testSimpleCalls() {
        const absF = new FFI.CFunction(libm.symbol('abs'), FFI.types.sint, [
            FFI.types.sint,
        ]);
        ...
        if (tjs.platform !== 'windows') {
            ...
            const fabsfF = new FFI.CFunction(
                libm.symbol('fabsf'),
                FFI.types.float,
                [FFI.types.float]
            );
        ...
        const atoiF = new FFI.CFunction(libc.symbol('atoi'), FFI.types.sint, [
            FFI.types.string,
        ]);
        assert.eq(atoiF.call('1234'), 1234);

        const strerrorF = new FFI.CFunction(
            libc.symbol('strerror'),
            FFI.types.string,
            [FFI.types.sint]
        );
        assert.eq(strerrorF.call(1 /* EPERM */), 'Operation not permitted');

        const sprintfF3 = new FFI.CFunction(
            libc.symbol('sprintf'),
            FFI.types.sint,
            [FFI.types.buffer, FFI.types.string, FFI.types.sint],
            1
        );
        ...
        const strcatF = new FFI.CFunction(
            libc.symbol('strcat'),
            FFI.types.string,
            [FFI.types.buffer, FFI.types.string]
        );
    ...
    function testSimpleVariables() {
        ...
        const testIntPointer = new FFI.Pointer(
            testIntSymbol.addr,
            1,
            FFI.types.sint
        );
        ...
        const testIntPtrPointer = new FFI.Pointer(
            testIntPtrSymbol.addr,
            2,
            FFI.types.sint
        );
    ...
    function testStructs() {
        ...
        const divF = new FFI.CFunction(libc.symbol('div'), divT, [
            FFI.types.sint,
            FFI.types.sint,
        ]);
    ...
    function testPointersAndStructsOpendir() {
        ...
        const opendirF = new FFI.CFunction(
            libc.symbol('opendir'),
            FFI.types.pointer,
            [FFI.types.string]
        );
        ...
        if (tjs.platform == 'darwin') {
            // macos has another dirent definition
        ...
        const readdirF = new FFI.CFunction(libc.symbol('readdir'), direntPtrT, [
            FFI.types.pointer,
        ]);
        const closedirF = new FFI.CFunction(
            libc.symbol('closedir'),
            FFI.types.sint,
            [FFI.types.pointer]
        );
    ...
    function testPointersAndStructsTime() {
        ...
        const timeF = new FFI.CFunction(libc.symbol('time'), FFI.types.sint64, [
            FFI.types.pointer,
        ]);
        ...
        const localtimeF = new FFI.CFunction(
            libc.symbol('localtime'),
            new FFI.PointerType(tmT, 1),
            [new FFI.PointerType(FFI.types.sint64, 1)]
        );
        const tmPtr = localtimeF.call(
            FFI.Pointer.createRef(FFI.types.sint64, testTimestamp)
        ); // test with 2022-07-20T14:16:27+02:00
        ...
        assert.eq(
            tm.isdst,
            cmpDate.getTimezoneOffset() !=
                new Date(2022, 1, 1, 1, 1, 1).getTimezoneOffset()
                ? 1
                : 0
    ...
    function testJsCallback() {
        const testlib = new FFI.Lib(sopath);
        const callCallbackF = new FFI.CFunction(
            testlib.symbol('call_callback'),
            FFI.types.sint,
            [FFI.types.jscallback, FFI.types.sint]
        );
        let recv = null;
    ...
    function testLibFromCProto() {
        ...
        assert.truthy(
            libc
                .call('asctime', FFI.Pointer.createRefFromBuf(structTmT, tmBuf))
                .match(regex)
        );
        assert.truthy(
            libc
                .call('asctime', FFI.Pointer.createRef(structTmT, tmData))
                .match(regex)
        );
    }
