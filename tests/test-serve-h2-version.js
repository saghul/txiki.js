...
// The server reports HTTP/2 (info.httpVersion === '2') for a request over TLS.
//
...
async function testServerSeesH2() {
    await withServer(
        (req, info) => new Response(info.httpVersion),
        async port => {
            const r = await fetch(`https://127.0.0.1:${port}/`, { allowInsecure: true });

            assert.eq(r.status, 200, 'status is 200');
            assert.eq(await r.text(), '2', 'server served the request over HTTP/2');
        },
    );
}

await testServerSeesH2();
