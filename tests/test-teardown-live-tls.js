...
// Regression test: an uncaught top-level exception with a live tls handle
...
const args = [
    tjs.exePath,
    'run',
    path.join(import.meta.dirname, 'helpers', 'teardown-live-tls.js')
];
const proc = tjs.spawn(args, { stdout: 'ignore', stderr: 'pipe' });
const [ status, stderrStr ] = await Promise.all([ proc.wait(), proc.stderr.text() ]);
assert.ok(stderrStr.includes('uncaught with a live TLS read'), 'child threw the expected error');
